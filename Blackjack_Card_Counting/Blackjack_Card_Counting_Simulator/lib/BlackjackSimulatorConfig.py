from blackjack import Blackjack  # pylint: disable=import-error
from players import Player
import argparse
import textwrap

class BlackjackSimulatorConfig:
    def __init__(self, player):
        # Set default values, these could be adjusted in the GUI
        self.players = 1
        self.bankroll = 1000
        self.decks = 6
        self.player = player
    ...
    def parse(self, simulation=False):
        ...
        if simulation:
            parser = self.parse_simulation_params(parser)

        # Receive the args
        args = parser.parse_args()
        vars(args)['bankroll'] = self.player.bankroll
        # Print ruleset
        ruleset_str = "\n".join(f"{key}: {value}" for key, value in vars(args).items() if key in ['players','bankroll','decks','penetration','blackjack_payout'])

        # Create the blackjack game to return
        if simulation:
            blackjack = self.create_blackjack(args, sim=True)
        else:
            blackjack = self.create_blackjack(args)
            
        # Possible additional rulesets:
        # https://wizardofodds.com/games/blackjack/calculator/

        return args, blackjack, ruleset_str
    ...
    def create_blackjack(self, args, sim=False):
        player_list = ['Player {}'.format(i+1) for i in range(args.players)]

        if hasattr(args, 'min_bet'):
